 ...
 * @author Alessio
 */
@WebFilter(filterName = "PazienteVisiteFilter", servletNames = {"PazienteVisite"}, dispatcherTypes = {DispatcherType.REQUEST, DispatcherType.FORWARD})
public class PazienteVisiteFilter implements Filter {
    
    private static final boolean DEBUG = false;
    
    private FilterConfig filterConfig = null;
    
    public PazienteVisiteFilter() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("PazienteVisiteFilter:DoBeforeProcessing");
        }
        ...
        if (sessione != null) {
            if (DEBUG) System.out.println("PazienteVisiteFilter:        sessione not null.");
            utente = (Utente) sessione.getAttribute("utente");
            paziente = (Paziente) sessione.getAttribute("paziente");
        }
        if (utente != null) {
            if (DEBUG) System.out.println("PazienteVisiteFilter:        utente not null.");
            utenteId = utente.getUtenteId();
        }
        
        if (utenteId == null) {
            if (DEBUG) System.out.println("PazienteVisiteFilter:        utenteId null.");
        ...
        try {
            if (utente == null) {
                if (DEBUG) System.out.println("PazienteVisiteFilter:              utente null.");
                utente = utenteDao.getByPrimaryKey(utenteId);
            }
            if (paziente == null) {
                if (DEBUG) System.out.println("PazienteVisiteFilter:              paziente null.");
                paziente = pazienteDao.getByPrimaryKey(utenteId);
            }
        } catch (DAOException ex) {
            throw new RuntimeException(new ServletException("Impossibile recuperare l'utente per le visite", ex));
        }
        request.setAttribute("utente", utente);
        request.setAttribute("paziente", paziente);
        
        try {
            List<Visita> visite = pazienteDao.getAllVisiteByPaziente(paziente);
            List<Visita> visiteEseguite = pazienteDao.getEVisiteByPaziente(paziente);
            List<Visita> visiteNonEseguite = pazienteDao.getNEVisiteByPaziente(paziente);
            ...
            throw new RuntimeException(new ServletException("Impossibile recuperate le visite", ex));
        }
        if (response.isCommitted()) {
            request.getServletContext().log("paziente/visite.html is already committed");
        }
    }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("PazienteVisiteFilter:DoAfterProcessing");
        }
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (DEBUG) {
            log("PazienteVisiteFilter:doFilter()");
        }
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (DEBUG) {                
                log("PazienteVisiteFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("PazienteVisiteFilter()");
        }
        StringBuffer sb = new StringBuffer("PazienteVisiteFilter(");
