 ...
 * @author Alessio
 */
public class MedicoPazientiFilter implements Filter {
    
    private static final boolean DEBUG = false;

    private FilterConfig filterConfig = null;
    
    public MedicoPazientiFilter() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("MedicoPazientiFilter:DoBeforeProcessing");
        }
        ...
        if (sessione != null) {
            if (DEBUG) System.out.println("MedicoPazientiFilter:       sessione not null.");
            utente = (Utente) sessione.getAttribute("utente");
        }
        if (utente != null) {
            if (DEBUG) System.out.println("MedicoPazientiFilter:       utente not null.");
            utenteId = utente.getUtenteId();
        }
        
        if (utenteId == null) {
            if (DEBUG) System.out.println("MedicoPazientiFilter:       utenteId null.");
        ...
        try {
            if (utente == null) {
                if (DEBUG) System.out.println("MedicoPazientiFilter:       utente null.");
        ...
        try {
            ...
            if(medico.getSpecializzazione().getSpecId() == 0) {
                pazienti = medicoDao.getPazientiByMedic(medico);
            }else {
                pazienti = medicoDao.getAllPazienti(medico);
            }
            request.setAttribute("pazienti", pazienti);
        } catch (DAOException ex) {
            throw new RuntimeException(new ServletException("Impossibile recuperate il medico o i pazienti per la lista dei pazieti", ex));
        }
        if (response.isCommitted()) {
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (DEBUG) {                
                log("MedicoPazientiFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("MedicoPazientiFilter()");
        }
        StringBuffer sb = new StringBuffer("MedicoPazientiFilter(");
