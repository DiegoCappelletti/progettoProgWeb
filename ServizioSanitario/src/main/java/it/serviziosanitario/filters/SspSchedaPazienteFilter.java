 ...
 * @author Alessio
 */
public class SspSchedaPazienteFilter implements Filter {
    
    private static final boolean DEBUG = false;

    private FilterConfig filterConfig = null;
    
    public SspSchedaPazienteFilter() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("SspSchedaPazienteFilter:DoBeforeProcessing");
        }
        ...
        try {
            ...
            throw new RuntimeException(new ServletException("Impossibile recuperare il dao factory per l'ssps", ex));
        }
        ...
        if (sessione != null) {
            if (DEBUG) System.out.println("SspSchedaPazienteFilter: sessione not null.");
        ...
        if (utente != null) {
            if (DEBUG) System.out.println("SspSchedaPazienteFilter:          utente not null.");
            utenteId = utente.getUtenteId();
        }
        if (utenteId == null) {
            if (DEBUG) System.out.println("SspSchedaPazienteFilter: utenteId null.");
        ...
        try {
            if (utente == null) {
                if (DEBUG) System.out.println("SspSchedaPazienteFilter: utente null.");
        ...
        Ssp ssp = null;
        try {
            ssp = sspDao.getByPrimaryKey(utenteId);
        } catch (DAOException ex) {
        ...
        try {
            ...
            Paziente paziente = sspDao.getPazienteByPrimaryKey(pazienteId);
            request.setAttribute("paziente", paziente);

            List<Esame> esami = sspDao.getAllEsamiByPaziente(paziente);
            List<Esame> esamiEseguiti = sspDao.getEEsamiByPaziente(paziente);
            List<Esame> esamiNonEseguiti = sspDao.getNEsamiByPaziente(paziente);
        ...
        if (response.isCommitted()) {
            request.getServletContext().log("ssp/scheda-paziente-ssp.html is already committed");
        }
    }    
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (DEBUG) {
            log("SspSchedaPazienteFilter:doFilter()");
        }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("SspSchedaPazienteFilter()");
        }
        StringBuffer sb = new StringBuffer("SspSchedaPazienteFilter(");
