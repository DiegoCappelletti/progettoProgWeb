 ...
 * @author Alessio
 */
public class SspProfiloFilter implements Filter {
    
    private static final boolean DEBUG = false;

    private FilterConfig filterConfig = null;
    
    public SspProfiloFilter() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("SspProfiloFilter:DoBeforeProcessing");
        }
        ...
        if (sessione != null) {
            if (DEBUG) System.out.println("SspProfiloFilter:        sessione not null.");
            utente = (Utente) sessione.getAttribute("utente");
        }
        if (utente != null) {
            if (DEBUG) System.out.println("SspProfiloFilter:        utente not null.");
            utenteId = utente.getUtenteId();
        }
        
        if (utenteId == null) {
            if (DEBUG) System.out.println("SspProfiloFilter:        utenteId null.");
        ...
        try {
            if (utente == null) {
                if (DEBUG) System.out.println("SspProfiloFilter:        utente null.");
        ...
        try {
            ...
            sessione.setAttribute("ssp", ssp);
        } catch (DAOException ex) {
            throw new RuntimeException(new ServletException("Impossibile recuperate l'ssp per il profilo", ex));
        }
        if (response.isCommitted()) {
            request.getServletContext().log("ssp/profilo.html is already committed");
        }
    }    
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (DEBUG) {
            log("SspProfiloFilter:DoAfterProcessing");
        }
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (DEBUG) {                
                log("SspProfiloFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("SspProfiloFilter()");
        }
        StringBuffer sb = new StringBuffer("SspProfiloFilter(");
